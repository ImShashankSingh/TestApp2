    ...
    internal class RestAPI
    {
        ...
        private static RestResult<T> ErrorResult<T>(string message, int statusCode)
        {
            if (statusCode == 403)
            {
                return new RestResult<T>
                {
                    Message = Messages.NotAuthorizedMessage,
        ...
        public static async Task<IRestResult<T>> GetAsync<T>(string uri, Dictionary<string, string> parameters, CancellationToken cancellationToken = default(CancellationToken))
        {
            try
            {
                timer = new Timer(OnTimerCallback, null, 3000, 3000);
                cancellationTokenSource = new CancellationTokenSource();
                if (parameters != null)
                {
